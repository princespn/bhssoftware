...
<?php echo $this->Session->flash(); ?>
 <hr>
<h1 class="sub-header"><?php __('Amazon German Database.');?> </h1>
  ...
  <div class="panel panel-default">
    <div class="panel-body">
      <div class="row">
        ...
        <?php echo $this->Html->link(__('Import Listing', true), array('controller' => 'german_listings', 'action' => 'import'),array('class' => 'btn btn-info btn-sm')); ?>
    ...
    <table class="table table-bordered table-striped table-hover">
      <thead>        
        <tr>
          ...
          <th><?php __('Category');?><select id="GermanListingCategory" name="data[GermanListing][category]">
                        <option value='--Select Category--'>--Select Category--</option>
<?php $option = $this->requestAction('/german_listings/categoriesPro'); //echo $this->Form->select('category',array($option)); 
...
echo '<option'.' '.$select.' '.'value='.$option.'>'.$option.'</option>';
    ...
    <tr>
    <td><?php $productid = $german_listing['GermanListing']['id'];if(!empty($german_listing['GermanListing']['error'])){$class ='checkerror';}else{$class ='checkbox1';}
echo $this->Form->input('GermanListing.id',array('class'=>$class, 'selected'=>'selected','label'=>'','multiple' => 'checkbox', 'value' =>$productid,'name'=>'checkid[]', 'type'=>'checkbox')); ?><?php if(!empty($german_listing['GermanListing']['error'])){echo "&#8595;";} ?></td>
                ...
                <td><?php echo $german_listing['GermanProductListing']['product_code']; ?></td>
                <td><?php echo $german_listing['GermanListing']['item_sku']; ?></td>
                <td><?php if (!empty($german_listing['GermanProductListing']['category'])){ echo $german_listing['ProductListing']['category'];} ?></td>
                <td><?php if(($german_listing['GermanListing']['recommended_browse_nodes1'])!=($german_listing['GermanListing']['recommended_browse_nodes1']))
{echo "<div style='color:red;'>Browse nodes is did not match master database.</div>";
                ...
                <td><?php
	if(((isset($wordlist[1]))!=='FBA') && (empty($german_listing['GermanListing']['item_name'])))
	{
...
?></td>
<td><?php 
$saleprice = $german_listing['GermanListing']['standard_price'];
if(!(empty($saleprice)))
{
	echo $saleprice;
	echo "</BR>";
	...
	if(isset($pieces[1])===($german_listing['GermanListing']['item_sku'])){
...
else if(($german_listing['GermanListing']['parent_child'])==='parent')
{
...
?></td>	
<td><?php 
$stanprice = $german_listing['GermanListing']['sale_price'];
if(!(empty($stanprice)))
{
	echo $stanprice;
	echo "</BR>";
...
<script type="text/javascript">
    document.getElementById("GermanListingCategory").onchange = function () {
...
$(document).ready(function() {
    $('#GermanListingError').click(function(event) {  //on click
    ...
    $('#selecctall').click(function(event) {  //on click
        if(this.checked) { // check select status
            $('.checkbox1').each(function() { //loop through each checkbox
                this.checked = true;  //select all checkboxes with class "checkbox1" 
				 $('div.btnClick').show();
				 $('#GermanListingError').attr('disabled','disabled');
            });
			 $('.checkerror').each(function() { //loop through each checkbox
                this.checked = true;  //select all checkboxes with class "checkbox1" 
				 $('div.btnClick').show();
            ...
            $('.checkbox1').each(function() { //loop through each checkbox
                this.checked = false; //deselect all checkboxes with class "checkbox1"                      
				$('div.btnClick').hide();
				$('#GermanListingError').removeAttr('disabled','disabled');
            }); 
			$('.checkerror').each(function() { //loop through each checkbox
                this.checked = false; //deselect all checkboxes with class "checkbox1"                      
				$('div.btnClick').hide();
