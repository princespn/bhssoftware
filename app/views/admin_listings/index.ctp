...
<?php echo $this->Session->flash(); ?>
 <hr>
<h1 class="sub-header"><?php __('Master Website Price Listing');?> </h1>
     ...
     <?php foreach ($price_listings as $price_listing): ?>
        <tr>
         ...
         <td><?php echo $price_listing['InventoryCode']['category']; ?></td>
          ...
          <td><?php echo $price_listing['AdminListing']['web_price_de']; ?></td>
          <td><?php echo $price_listing['AdminListing']['web_sale_price_de']; ?></td>
          <td><?php echo $price_listing['AdminListing']['web_price_fr']; ?></td>
          <td><?php echo $price_listing['AdminListing']['web_sale_price_fr']; ?></td>          
           <td><?php echo $price_listing['AdminListing']['web_sale_price_tesco']; ?></td>
           <td><?php echo $price_listing['AdminListing']['web_price_dm']; ?></td>
           <td><?php echo $price_listing['AdminListing']['web_sale_price_dm']; ?></td>
    <?php if($session->read('Auth.User.group_id')!='3') { ?>
    <td><?php echo $this->Html->link('<i aria-hidden="true" class="fa fa-edit"></i>',array('controller'=>'admin_listings','action'=>'edit', $productid),array('class'=> 'edit-btn','escape'=>false)); echo $this->Html->link('<i aria-hidden="true" class="fa fa-close"></i>', array('controller'=>'admin_listings','action' => 'delete',$productid), array('class'=> 'delete-btn','escape' => false), sprintf(__('Are you sure you want to delete # %s?', true), $price_listing['AdminListing']['web_sku']));  ?></td><?Php } ?>
         </tr>        
        <?php endforeach; ?>
      </tbody>
    </table>
