				  ...
				  <?php foreach ($getCost as $exchange_rate): ?>
                                    ...
                                    <td><?php       $amount = "1"; $from = $exchange_rate['CostSetting']['invoice_currency']; $to =  $exchange_rate['CostSetting']['sale_base_currency'];
                                                         ...
                                                         $data = file_get_contents($url);
                                                          ...
                                                          $ExRate = round($converted, 2); echo $ExRate;?></td>
                                      </tr>
                                 <?php endforeach; ?> 
			</table>
		</div>
...
<div class="table-responsive">
    <table class="table table-bordered table-striped table-hover">
      ...
      <?php foreach ($purchase_orders as $purchase_order): ?>
         ...
         echo $this->Form->input('PurchaseOrder.id',array('class'=>$class,'label'=>'','multiple' => 'checkbox', 'value' =>$pid,'name'=>'checkid[]', 'type'=>'checkbox')); ?> <?php if(!empty($purchase_order['PurchaseOrder']['error'])){echo "&#8595;";} ?></td>
