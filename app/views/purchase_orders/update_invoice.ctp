        ...
        <tr> 
         ...
         echo $this->Form->input('PurchaseOrder.id',array('class'=>$class, 'selected'=>'selected','label'=>'','multiple' => 'checkbox', 'value' =>$pid,'name'=>'checkid[]', 'type'=>'checkbox')); ?> <?php if(!empty($purchase_order['PurchaseOrder']['error'])){echo "&#8595;";} ?></td>
          ...
          <td><?php echo $purchase_order['PurchaseOrder']['product_name']; ?></td>          
            <?php if(($purchase_order['PurchaseOrder']['latest_invoice']) !== ($purchase_order['PurchaseOrder']['invoice_value']) && (($purchase_order['PurchaseOrder']['id'])=== $invoiceid)) { ?>
            <?php  echo $form->create('PurchaseOrder',array('action'=>'update_invoice','id'=>'updateForm')); ?>
         ...
         <?php } ?>
          ...
          <td><?php // Currency Master Information in GBP---                   
           ...
           <?php   endif; endforeach; ?>                  
            <?php if(((!empty($purchase_order['AdminListing']['web_sale_price_uk'])) && (!empty($purchase_order['PurchaseOrder']['sale_price_uk']))) && (($purchase_order['AdminListing']['web_sale_price_uk'])===($purchase_order['PurchaseOrder']['sale_price_uk']))) {    ?>
            <td><?php echo $Webprice['AdminListing']['web_sale_price_uk']; ?></td>
            <?php } else { ?><td class="red-info" title="<?php Echo "Selling Price GBP :: ".$purchase_order['PurchaseOrder']['sale_price_uk']." Web Price GBP :: ".$purchase_order['AdminListing']['web_sale_price_uk']." Mismatch."; ?>"><?php echo $purchase_order['AdminListing']['web_sale_price_uk']; ?></td> 
            <?php } ?>            
