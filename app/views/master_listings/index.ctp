...
<?php foreach ($code_listings as $code_listing): ?>
       ...
       <?php if(($code_listing['MasterListing']['linnworks_code']) === ($Amazondeukfba['MasterListing']['linnworks_code'])){  ?>  
        ...
        <?php  $gbpCountry = array(); $gbpCategory = array(); $gbpCost = array(); foreach ($getshipppings as $getdeshiping): ?>
              <?php if((($getdeshiping['Shipping']['country']) ==='GB') && ($getdeshiping['Shipping']['category']) === ($code_listing['InventoryCode']['category'])):  ?>
                <?php $gbpCountry[0] = $getdeshiping['Shipping']['country'];
                $gbpCategory[0] = $getdeshiping['Shipping']['category'];
               $gbpCost[0] = $getdeshiping['Shipping']['shipping_cost']; ?>
        <?php   break;endif; endforeach; ?>
        
         <td><?php foreach ($Amazonuk as $Amazondeukfba): ?>
        <?php if(($code_listing['MasterListing']['linnworks_code']) === ($Amazondeukfba['MasterListing']['linnworks_code'])){  ?>  
        <?php if((strpos($Amazondeukfba['MasterListing']['amazon_sku'], 'FBA') !== false) && (($code_listing['InventoryCode']['category'] === $gbpCategory[0]) && ($gbpCountry[0] === 'GB')) && ((!empty($Amazondeukfba['MasterListing']['sale_price_uk'])) && (!empty($code_listing['AdminListing']['web_sale_price_uk']))) && ((int)$Amazondeukfba['MasterListing']['sale_price_uk'] !==(int)$code_listing['AdminListing']['web_sale_price_uk']+(int)$gbpCost[0])){  ?>
       ...
       <?php if(($code_listing['MasterListing']['linnworks_code']) === ($amazondefba['MasterListing']['linnworks_code'])){  ?>  
        ...
        <?php endforeach; ?></td> 
          ...
          <?php   $debCountry = array();$debCategory = array();$debCost = array(); foreach ($getshipppings as $getshippping): ?>
              <?php if((($getshippping['Shipping']['country']) ==='DE') && ($getshippping['Shipping']['category']) === ($code_listing['InventoryCode']['category'])):  ?>
                <?php $debCountry[0] = $getshippping['Shipping']['country'];
                $debCategory[0] = $getshippping['Shipping']['category'];
               $debCost[0] = $getshippping['Shipping']['shipping_cost']; ?>
        <?php   break;endif; endforeach; ?>
        
        <td><?php foreach ($Germany_primes as $amazondefba): ?>
        <?php if(($code_listing['MasterListing']['linnworks_code']) === ($amazondefba['MasterListing']['linnworks_code'])){  ?>  
        <?php if((strpos($amazondefba['MasterListing']['amazon_sku'], 'FBA') !== false) && (($code_listing['InventoryCode']['category'] === $debCategory[0]) && ($debCountry[0] === 'DE')) && ((!empty($amazondefba['MasterListing']['sale_price_de'])) && (!empty($code_listing['AdminListing']['web_sale_price_de']))) && ((int)$amazondefba['MasterListing']['sale_price_de'] !==(int)$code_listing['AdminListing']['web_sale_price_de']+(int)$debCost[0])){  ?>
       ...
       <?php if(($code_listing['MasterListing']['linnworks_code']) === ($amazonfrfba['MasterListing']['linnworks_code'])){  ?>  
        ...
        <?php endforeach; ?></td> 
           ...
           <?php  $frbCountry = array();$frbCategory = array();$frbCost = array(); foreach ($getshipppings as $getfrshippping): ?>
              <?php if((($getfrshippping['Shipping']['country']) ==='FR') && ($getfrshippping['Shipping']['category']) === ($code_listing['InventoryCode']['category'])):  ?>
                <?php $frbCountry[0] = $getfrshippping['Shipping']['country'];
                $frbCategory[0] = $getfrshippping['Shipping']['category'];
               $frbCost[0] = $getfrshippping['Shipping']['shipping_cost']; ?>
        <?php   break;endif; endforeach; ?>
