...
<?php echo $this->Session->flash(); ?>
 <hr>
<h1 class="sub-header"><?php __('Amazon France Listing');?> </h1>
  ...
  <div class="panel panel-default">
    <div class="panel-body">
      <div class="row">
        ...
        <?php echo $this->Html->link(__('Import Listing', true), array('controller' => 'france_listings', 'action' => 'import'),array('class' => 'btn btn-info btn-sm')); ?>
...
echo '<option'.' '.$select.' '.'value='.$option.'>'.$option.'</option>';
      ...
      <tbody>
          <?php $output = $this->requestAction('/france_listings/saleprice'); $keywords = preg_split("/[\n]+/", $output); ?>
          <?php foreach ($france_listings as $france_listing): ?>
          <?php $wordlist = split ("\_", $france_listing['FranceListing']['item_sku']);  ?>
        <tr>
          ...
          <td><?php  if(!empty($france_listing['FranceListing']['main_image_url'])){	echo "<img width='70px' src=".$france_listing['FranceListing']['main_image_url'].">";
          ...
          <td><?php if (!empty($france_listing['FranceProductListing']['category'])){ echo $france_listing['FranceProductListing']['category'];} ?></td>
           <td><?php if(($france_listing['FranceListing']['recommended_browse_nodes1'])!=($france_listing['FranceListing']['recommended_browse_nodes1'])){echo "<div style='color:red;'>Browse nodes is did not match master database.</div>";}else{echo $france_listing['FranceListing']['recommended_browse_nodes1'];} ?></td>
           <td><?php	if(((isset($wordlist[1]))!=='FBA') && (empty($france_listing['FranceListing']['item_name']))){echo "<div style='color:red;'>The Title is required</div>";}else{$row1 = $france_listing['FranceListing']['item_name'];$item = strlen($row1); if($item >= '500'){	echo "<div style='color:red;'>The Title must be no long 500 characters.</div>";	}else{	$itemname = utf8_encode(substr($row1,0,50)); echo ($itemname);}}?></td>
            <td><?php $saleprice = $france_listing['FranceListing']['standard_price'];if(!(empty($saleprice))){
	echo $saleprice;
	echo "</BR>";
       
	foreach ($keywords as $keyword){
	$pieces = explode(",", $keyword);       
	if(isset($pieces[1])===($france_listing['FranceListing']['item_sku'])){
...
?></td>
 <td><?php $stanprice = $france_listing['FranceListing']['sale_price'];if(!(empty($stanprice))){echo $stanprice;echo "</BR>";foreach ($keywords as $keyword){$pieces = explode(",", $keyword);if((isset($pieces[1]))===($france_listing['FranceListing']['item_sku'])){if((is_int(isset($pieces[3]))) !== (is_int($stanprice)) || (is_float(isset($pieces[3]))) !== (is_float($stanprice))){echo "<span style='color:red;'>Sale Price is did not match.</span>";}}}}else if(($france_listing['FranceListing']['parent_child'])==='parent'){echo "<span style='color:red;'>Parent</span>";}else{echo "<span style='color:red;'>Sale Price is Required</span>";}?></td>      
    <td><?php echo $this->Html->link('<i aria-hidden="true" class="fa fa-edit"></i>',array('controller'=>'$france_listings','action'=>'edit', $france_listing['FranceListing']['item_sku']),array('class'=> 'edit-btn','escape'=>false)); echo $this->Html->link('<i aria-hidden="true" class="fa fa-close"></i>', array('controller'=>'$france_listings','action' => 'delete', $france_listing['FranceListing']['item_sku']), array('class'=> 'delete-btn','escape' => false), sprintf(__('Are you sure you want to delete # %s?', true), $france_listing['FranceListing']['item_sku']));  ?></td>
       </tr>        
        <?php endforeach; ?>
      </tbody>
    </table>
...
<script type="text/javascript">
    document.getElementById("FranceListingCategory").onchange = function () {
