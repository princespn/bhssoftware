...
?>
 <?php //print_r($Countskulastweeks);die(); ?>
  <h1 class="sub-header"><?php __('Sales Products SKU Weekly Orders Reports');?></h1>
 <div class="panel panel-default">
    <div class="panel-body">
      <div class="row">
               ...
               <?php // $Catname = $this->requestAction('/master_listings/categories'); // print_r($option); die(); ?>
                  <ul class="dropdown-menu">
                   <?php foreach ($categories as $Catna): ?>    
                     <li><a href="<?php echo  $actual_link ; ?>/processed_listings/productsku_weekly/<?php echo rawurlencode($Catna->CategoryName); ?>" target="_self"><?php echo $Catna->CategoryName; ?></a></li>
                 <?php endforeach; ?>
                </ul>
              </li>
            </ul>         
       ...
       <?php  echo $form->create('ProcessedListing',array('action'=>'productsku_weekly')); ?>
        <div class="col-md-4">
  ...
  <div class="table-responsive">
    <table class="table table-bordered table-striped table-hover">
       ...
       <tr><td class='width55' colspan="5"><?php __('Product sku');?></td><td><?php __('No of Orders');?></td><td><?php __('Currency');?></td><?php echo $rowcolvalue; ?><td><?php __('No of Orders');?></td><td><?php __('Order Progress');?></td><td><?php __('Currency');?></td><?php echo $rowcolvalue; ?><td><?php __('Value Progress');?></td><td><?php __('No of Orders');?></td><td><?php __('Order Progress');?></td><td><?php __('Currency');?></td><?php echo $rowcolvalue; ?><td><?php __('Value Progress');?></td></tr>
        <?php    $a = '0'; foreach ($saveallproductskus as $value): ?>  
        <?php $b = $value['ProcessedListing']['product_sku']; $catname = substr($value['ProcessedListing']['product_name'],0,20); $fulltitle = substr($value['ProcessedListing']['product_name'],0,-1);?> 
         <?php  $numcurgbp = array();$ordercurgbp = array(); $numcureur = array(); $ordercureur = array(); foreach ($Countskucurrweeks as $CatGbpweek): ?> 
        ...
        <?php  $numprevgbp = array();$orderprevgbp = array(); $numpreveur = array(); $orderpreveur = array();  foreach ($Countskuprevweeks as $Countprevweek): ?>       
        ...
        <?php  $numlastgbp = array();$orderlastgbp = array(); $numlasteur = array(); $orderlasteur = array();  foreach ($Countskulastweeks as $Countlastweek): ?>       
       ...
       <?php if(($session->read('Auth.User.group_id')!=='5')){ $order1 = "<td class='width15'>". round($ordercurgbp[0],2) ."</td>"; $order2 = "<td>". round($ordercureur[0],2)  ."</td>";  $order3 = "<td class='width20'>". round($orderprevgbp[0],2) ."</td>"; $order4 = "<td class='width20'>". round($orderpreveur[0],2) ."</td>"; $order5 = "<td class='width20'>". round($orderlastgbp[0],2) ."</td>"; $order6 = "<td class='width20'>". round($orderlasteur[0],2) ."</td>"; } else { } ?>
        ...
        <?php  if((!empty($ordercureur[0])) && (!empty($orderlasteur[0]))){ $lastvalueeurprogress = ((($ordercureur[0]/$orderlasteur[0])-1)*100);  if($lastvalueeurprogress < 0){$lastvalueeurprog = "<td class='width20 red'>".round($lastvalueeurprogress,2)."%"."</td>"; }else { $lastvalueeurprog = "<td class='width20 green'>".round($lastvalueeurprogress,2)."%"."</td>"; } } else { $lastvalueeurprog = "<td class='width20 green'>-</td>"; }?>
         <?php if($a!==$b){ echo "<tr><td colspan='22'><div class='accordion sale-by-category'><table class='table-responsive category'><tr><td class='width33'><table class='table-responsive table-bordered'><tr><td colspan='5' class='width33'>".$b."</td><td class='width15'>". $numcurgbp[0] ."</td><td class='width15'>GBP</td>". $order1 ."</tr><tr><td colspan='5' title='". $fulltitle ."'  class='width33'>". $catname ."</td><td>". $numcureur[0] ."</td><td>EUR</td>". $order2 ."</tr></table></td><td class='width33'><table class='table-responsive table-bordered'><tr><td class='width20'>". $numprevgbp[0] ."</td>". $currnumprogress ."<td class='width20'>GBP</td>". $order3 . $curvaluegbpprog ."</tr><tr><td class='width20'>". $numpreveur[0] ."</td>". $cureurnumprogress ."<td class='width20'>EUR</td>". $order4 . $curvalueeurprog ."</tr></table><td class='width33'><table class='table-responsive table-bordered'><tr><td class='width20'>". $numlastgbp[0] ."</td>". $lastgbpprog ."<td class='width20'>GBP</td>". $order5 . $lastvaluegbpprog ."</tr><tr><td class='width20'>". $numlasteur[0] ."</td>". $lasteurprog ."<td class='width20'>EUR</td>". $order6 . $lastvalueeurprog ."</tr></table></tr></table></div>";} ?>
        ...
        <div class="rTableHeading"><div class="rTableHead"><?php __('Sales Platform');?></div>
           ...
           <div class="rTableRow"><div class="rTableCell"><?php echo $value['ProcessedListing']['plateform']; ?></div>
               <div class="rTableCell"><?php echo $value['ProcessedListing']['subsource']; ?></div>                                 
                                ...
                                <?php foreach ($productskucurrents as $currentweeks): ?>  
                                ...
                                <?php foreach ($productskupreviousweeks as $previousweeks): ?>  
                                ...
                                <div class='rTableCell'><?php if($pordercurreur[0]==='EUR'){echo $pordercurreur[0];}else if($pordercurregbp[0]==='GBP') {echo $pordercurregbp[0]; } else{echo "-";}  ?></div>         
                                 <?php if(($session->read('Auth.User.group_id')!='5')){ ?><div class='rTableCell'><?php if((!empty($pordervalueeur[0])) && ($pordercurreur[0]==='EUR')){  $porder[0] = $pordervalueeur[0]*0.89; echo  round($porder[0],2);   } else if ((!empty($pordervaluegbp[0])) && ($pordercurregbp[0]==='GBP')){echo round($pordervaluegbp[0],2); } else {echo "-";}?></div><?php } ?>
                                ...
                                <?php foreach ($productskulastsweeks as $datalastweek): ?>  
                               ...
                               <div class='rTableCell'><?php if($lastordercurreur[0]==='EUR'){echo $lastordercurreur[0];}else if($lastordercurrgbp[0]==='GBP') {echo $lastordercurrgbp[0]; } else{echo "-";}  ?></div>         
                                <?php if(($session->read('Auth.User.group_id')!='5')){ ?><div class='rTableCell'><?php if((!empty($lastordervalueeur[0])) && ($lastordercurreur[0]==='EUR')){  $lastorder[0] = $lastordervalueeur[0]*0.89; echo  round($lastorder[0],2);   } else if ((!empty($lastordervaluegbp[0])) && ($lastordercurrgbp[0]==='GBP')){echo round($lastordervaluegbp[0],2); } else {echo "-";}?></div><?php } ?>
