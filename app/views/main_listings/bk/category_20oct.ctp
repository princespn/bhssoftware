   ...
   <?php  echo $form->create('MainListing',array('action'=>'index')); ?>
    <div class="panel-body">
      <div class="row">
       ...
       <div class="col-md-3"><label for="<?php __('Category');?>"><?php __('Category : ');?></label><select id="MainListingCategory" name="data[MainListing][category]">
<option value='<?php __('--Select Category--');?>'><?php __('--Select Category--');?></option>
...
}?></select></div><div class="col-md-3"><?php  echo $this->Form->input('uk_min_price',array('type'=>'text','ng-model'=>'uk_min_price'));?></div><div class="col-md-3"><?php  echo $this->Form->input('uk_max_price',array('type'=>'text','ng-model'=>'uk_max_price')); ?></div><div class="col-md-3"><?php echo $this->Form->button('Apply', array('value'=>'Apply','name'=>'Apply','id'=>'Apply','class'=>'btn btn-primary','type'=>'submit')); ?></div>
    </div>      
    </div>
   </div>
    ...
    <table class="table table-bordered table-striped table-hover">
      <thead>
        ...
        <tr>
          ...
          <th class="wid-120"><ul class="select-drop">
...
<?php foreach ($code_listings as $code_listing): ?>
        <tr>
         ...
         echo $this->Form->input('MainListing.id',array('class'=>$class, 'selected'=>'selected','label'=>'','multiple' => 'checkbox', 'value' =>$productid,'name'=>'checkid[]', 'type'=>'checkbox')); ?> <?php if(!empty($code_listing['MainListing']['error'])){echo "&#8595;";} ?></td>
          ...
          <?php if((($desku[1])!=='FBA') && (($final) !== 'FBA') && (!empty($code_listing['MainListing']['price_uk'])) && (!empty($code_listing['Listing']['web_price_uk'])) && (($code_listing['MainListing']['price_uk'])!==($code_listing['Listing']['web_price_uk']))){  ?>
          ...
          <td><?php echo $code_listing['Listing']['web_sale_price_tesco']; ?></td>       
            <?php if(((($desku[1])!=='FBA') && (($final) !== 'FBA') && (!empty($code_listing['MainListing']['sale_price_uk'])) && (!empty($code_listing['Listing']['web_sale_price_tesco']))) && (($code_listing['MainListing']['sale_price_uk'])!==($code_listing['Listing']['web_sale_price_tesco']))){  ?>
            <td class="red-info" title="<?php Echo "Web UK :: ".$code_listing['Listing']['web_sale_price_uk']." Amazon Uk :: ".$code_listing['MainListing']['sale_price_uk']." Tesco web :: ".$code_listing['Listing']['web_sale_price_tesco']." Sale Price Mismatch."; ?>"><?php  echo $code_listing['MainListing']['sale_price_uk']; ?></td>
            <?php }else if(((($desku[1])!=='FBA') && (!empty($code_listing['MainListing']['sale_price_uk'])) && (!empty($code_listing['Listing']['web_sale_price_tesco'])) && (!empty($code_listing['Listing']['web_sale_price_uk']))) && (($code_listing['Listing']['web_sale_price_uk'])!==($code_listing['Listing']['web_sale_price_tesco']))){  ?>
            <td class="red-info" title="<?php Echo "Web UK :: ".$code_listing['Listing']['web_sale_price_uk']." Amazon Uk :: ".$code_listing['MainListing']['sale_price_uk']." Tesco web :: ".$code_listing['Listing']['web_sale_price_tesco']." Sale Price Mismatch."; ?>"><?php  echo $code_listing['MainListing']['sale_price_uk']; ?></td>
            <?php }else if(($desku[1])=='FBA') { ?><td></td>
          <?php }else{ ?>
            <td><?php  echo $code_listing['MainListing']['sale_price_uk']; ?></td>
          <?php } ?>
        <?php $uksku = split ("\_", $code_listing['MainListing']['amazon_sku']);  ?>  
        <?php if(($uksku[1]==='FBA') && (!empty($code_listing['MainListing']['sale_price_uk']))) {$val =$code_listing['MainListing']['sale_price_uk']; $min = $code_listing['Listing']['web_sale_price_uk']+3; $max = $code_listing['Listing']['web_sale_price_uk']+6;if($val >= $min && $val <= $max){ echo "<td>".$code_listing['MainListing']['sale_price_uk']."</td>";}else{echo "<td class='red-info'  title="."'Prim Min Price :: ".$min."  And Max Price :: ".$max."'>".$code_listing['MainListing']['sale_price_uk']."</td>";}}else {echo "<td></td>";}  ?>
        <?php  if(($uksku[1])==='FBA'){ ?>
        <?php if((!empty($code_listing['Listing']['web_sale_price_dm'])) && (($code_listing['Listing']['web_sale_price_dm'])===($code_listing['MainListing']['sale_price_uk']))) { ?>
        ...
        <?php } ?>
          ...
          <?php } ?>
         <?php if((!empty($code_listing['Listing']['web_sale_price_de']))) {$val =$code_listing['Listing']['web_sale_price_de']; $min = $code_listing['Listing']['web_sale_price_uk']*1.25; $max = $code_listing['Listing']['web_sale_price_uk']*1.6;if($val >= $min && $val <= $max){ echo "<td>".$code_listing['Listing']['web_sale_price_de']."</td>";}else{echo "<td class='red-info'  title="."'EUR Min Price :: ".$min."  And Max Price :: ".$max."'>".$code_listing['Listing']['web_sale_price_de']."</td>";}}else {echo "<td></td>";}  ?>
      
          <?php if((!empty($code_listing['MainListing']['sale_price_de']))) {$val =$code_listing['MainListing']['sale_price_de']; $min = $code_listing['Listing']['web_sale_price_uk']*1.25; $max = $code_listing['Listing']['web_sale_price_uk']*1.6;if($val >= $min && $val <= $max){ echo "<td>".$code_listing['MainListing']['sale_price_de']."</td>";}else{echo "<td class='red-info'  title="."'EUR Min Price :: ".$min."  And Max Price :: ".$max."'>".$code_listing['MainListing']['sale_price_de']."</td>";}}else {echo "<td></td>";}  ?>
          ...
          <?php } ?>
          <?php if((!empty($code_listing['MainListing']['sale_price_fr']))) {$val =$code_listing['MainListing']['sale_price_fr']; $min = $code_listing['Listing']['web_sale_price_uk']*1.25; $max = $code_listing['Listing']['web_sale_price_uk']*1.6;if($val >= $min && $val <= $max){ echo "<td>".$code_listing['MainListing']['sale_price_fr']."</td>";}else{echo "<td class='red-info'  title="."'EUR Min Price :: ".$min."  And Max Price :: ".$max."'>".$code_listing['MainListing']['sale_price_fr']."</td>";}}else {echo "<td></td>";}  ?>
