  ...
  <div class="table-responsive">
    <table class="table table-bordered table-striped table-hover">
      ...
      <tbody>
         ...
         <?php  $currdata[] = $value[0]['ordervalues']; ?>
           <tr>               
              <td><?php echo $value['ProcessedOrder']['subsource']; ?></td>
               ...
               <?php $Gerordersum+= $value[0]['orderid'];$Gersum+= $value[0]['ordervalues']; ?>
             </tr>
             <?php endforeach; ?> 
             <tr><td><?php __('Total Orders Num:');?></td><td><?php echo $Gerordersum; ?></td><td><?php __('Values:');?></td><td><?php echo round($Gersum,2); ?></td></tr>
             </tbody>
    </table>
            </td>
            <td>
      ...
      <tbody>
         <?php  foreach ($previousweeks as $previousweek): ?> 
           <?php  $prevdata[] = $previousweek[0]['ordervalues']; ?>
           <tr>               
               ...
               <?php $FRordersum+= $previousweek[0]['orderid']; $FRsum+= $previousweek[0]['ordervalues'];  ?>
             </tr>
             <?php endforeach; ?> 
              <tr><td><?php __('Total Orders Num:');?></td><td><?php echo $FRordersum; ?></td><td><?php __('Values:');?></td><td><?php echo round($FRsum,2); ?></td></tr>
          </tbody>
    </table>
            </td>
            <td>
      ...
      <tbody>
         <?php  foreach ($datalastweeks as $datalastweek): ?> 
            <?php  $lastdata[] = $datalastweek[0]['ordervalues']; ?>
           <tr>               
              <td><?php echo $datalastweek['ProcessedOrder']['subsource']; ?></td>
               ...
               <td><?php echo round($datalastweek[0]['ordervalues'],2); ?></td> 
                   <?php  $PRordersum+= $datalastweek[0]['orderid']; $PRsum+= $datalastweek[0]['ordervalues']; ?>
             </tr>
             <?php endforeach; ?> 
                  <tr><td><?php __('Total Orders Num:');?></td><td><?php echo $PRordersum; ?></td><td><?php __('Values:');?></td><td><?php echo round($PRsum,2); ?></td></tr>
            </tbody>
    </table>
            </td>
...
<script>
    new RGraph.Bar({
       ...
       //data: [ [16388.23,34769.09,18336.1], [5379.44,12557.31,4788.64], [395.72,1305.46,1120.84], [2975.02,6228.8,0], [1525.16,2683.23,1282.69], [339.37,1076.56,229.84], [3875.89,6687.26,3785.99], [1108.88,1541.74,927.41], [6404.85,15354.54,6862.62], [687.81,1149.48,1805.07] ],
          data:[ [<?php echo $currdata[0].','.$prevdata[0].','.$lastdata[0]; ?>],[<?php echo $currdata[1].','.$prevdata[1].','.$lastdata[1]; ?>],[<?php echo $currdata[3].','.$prevdata[4].','.$lastdata[0]; ?>],[<?php echo $currdata[4].','.$prevdata[5].','.$lastdata[3]; ?>],[<?php echo $currdata[5].','.$prevdata[6].','.$lastdata[4]; ?>],[<?php echo $currdata[6].','.$prevdata[7].','.$lastdata[5]; ?>],[<?php echo $currdata[7].','.$prevdata[8].','.$lastdata[6]; ?>],[<?php echo $currdata[8].','.$prevdata[9].','.$lastdata[7]; ?>],[<?php echo $currdata[9].','.$prevdata[10].','.$lastdata[8]; ?>],[<?php echo $currdata[10].','.$prevdata[11].','.$lastdata[9]; ?>] ],
          options: {
            textAccessible: true,
            variant: '3d',
            ...
            gutterRight: 15,
            gutterBottom: 10,
            labels: ['EBay',' Magento uk',' Tesco','Amazon Uk',' CDiscount',' Amazon Fr',' Amazon Gr',' Magento Fr',' Magento Gr',' Amazon Sp'],
            shadowColor:'#ccc',
            shadowOffsetx: 3,
