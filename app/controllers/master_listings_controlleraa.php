...
class MasterListingsController extends AppController {
    ...
    public function index() {
        ...
        $getshipppings   =	$this->Shipping->find('all',array('fields' => array('Shipping.*'),'recursive' => -1,$this->paginate()));

        $this->set('title', 'Master listing database.');
        //$condFba = (strpos('MasterListing.amazon_sku', 'FBA') !== false);

        $this->paginate = array('limit' => 100, 'order' => 'MasterListing.id');
        $Amazonuk   =	$this->MasterListing->find('all',array('fields' => array('MasterListing.*'),'recursive' => -1,$this->paginate()));
    ...
    public function index_prices() {
        ...
        if ((!empty($this->data)) && (!empty($_POST['submit'])) && (!empty($this->data['MasterListing']['all_item']))) {
            ...
            $this->MasterListing->recursive = 1;
            $this->paginate = array('limit' => 500, 'order' => 'MasterListing.id');
