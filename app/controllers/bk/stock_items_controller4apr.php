...
class StockItemsController extends AppController {
 ...
 public function categname() {
        ...
        return $catgory;
 		...
 		public function index(){
			...
			public function dailysales_reports(){
					...
					$currweek_Reports = $this->ProcessedListing->find('all',array('fields' => array('ProcessedListing.product_sku', 'ProcessedListing.product_name', 'ProcessedListing.cat_name','count(ProcessedListing.quantity) as sales_qty'), 'conditions' =>$currweek_condition, 'group' => $groupby, 'order' => array('count(ProcessedListing.quantity) desc'), 'limit' => 100));
         ...
         'AND'=> 'ProcessedListing.subsource');
					...
					$Reports = $this->ProcessedListing->find('all',array('fields' => array('ProcessedListing.product_sku', 'ProcessedListing.cat_name', 'ProcessedListing.plateform', 'ProcessedListing.product_name', 'ProcessedListing.subsource','count(ProcessedListing.quantity) as sales_qty'), 'conditions' =>$condition, 'group' => $groupby, 'order' => array('count(ProcessedListing.quantity) desc')));
					...
					$yes_Reports = $this->ProcessedListing->find('all',array('fields' => array('ProcessedListing.product_sku', 'ProcessedListing.plateform', 'ProcessedListing.product_name', 'ProcessedListing.subsource','count(ProcessedListing.quantity) as sales_qty'), 'conditions' =>$yes_condition, 'group' => $groupby, 'order' => array('count(ProcessedListing.quantity) desc')));
					...
					$lastseven_Reports = $this->ProcessedListing->find('all',array('fields' => array('ProcessedListing.product_sku', 'ProcessedListing.plateform', 'ProcessedListing.product_name', 'ProcessedListing.subsource','count(ProcessedListing.quantity) as sales_qty'), 'conditions' =>$lastseven_condition, 'group' => $groupby, 'order' => array('count(ProcessedListing.quantity) desc')));
					
					
						//print_r($lastseven_Reports);die();
					...
					$lasttherty_Reports = $this->ProcessedListing->find('all',array('fields' => array('ProcessedListing.product_sku', 'ProcessedListing.plateform', 'ProcessedListing.product_name', 'ProcessedListing.subsource','count(ProcessedListing.quantity) as sales_qty'), 'conditions' =>$lasttherty_condition, 'group' => $groupby, 'order' => array('count(ProcessedListing.quantity) desc')));
					//print_r();die();
					...
					$lastninty_Reports = $this->ProcessedListing->find('all',array('fields' => array('ProcessedListing.product_sku', 'ProcessedListing.plateform', 'ProcessedListing.product_name', 'ProcessedListing.subsource','count(ProcessedListing.quantity) as sales_qty'), 'conditions' =>$lastninty_condition, 'group' => $groupby, 'order' => array('count(ProcessedListing.quantity) desc')));
					//print_r();die();
					...
					$last365_Reports = $this->ProcessedListing->find('all',array('fields' => array('ProcessedListing.product_sku', 'ProcessedListing.plateform', 'ProcessedListing.product_name', 'ProcessedListing.subsource','count(ProcessedListing.quantity) as sales_qty'), 'conditions' =>$last365_condition, 'group' => $groupby, 'order' => array('count(ProcessedListing.quantity) desc')));
					//print_r();die();
		...
		//order value
			...
			public function salesvalue_reports(){
				
					$this->set('title', 'Daily Sales Reports - Top SKU  Ordered value.');
					...
					$sixmonth_Reports = $this->ProcessedListing->find('all',array('fields' => array('ProcessedListing.product_sku', 'ProcessedListing.product_name', 'ProcessedListing.cat_name','count(ProcessedListing.quantity) as sales_qty','sum(ProcessedListing.price_per_product) as sale_vale'), 'conditions' =>$six_condition, 'group' => $groupby, 'order' => array('sum(ProcessedListing.price_per_product) desc'), 'limit' => 100));
					
					$currweek_condition = array('ProcessedListing.order_date <= ' => $this_week_ed,
                    'ProcessedListing.order_date >= ' => $this_week_sd,'ProcessedListing.cat_name !='=>'','ProcessedListing.price_per_product !='=>'0','ProcessedListing.currency !='=>'','ProcessedListing.plateform !='=>'','ProcessedListing.subsource !='=>'http://bhsindia.com','ProcessedListing.subsource !='=>'','ProcessedListing.subsource !='=>'http://dev.homescapesonline.com');
                    
					$currweek_Reports = $this->ProcessedListing->find('all',array('fields' => array('ProcessedListing.product_sku', 'ProcessedListing.product_name', 'ProcessedListing.cat_name','count(ProcessedListing.quantity) as sales_qty','sum(ProcessedListing.price_per_product) as sale_vale'), 'conditions' =>$currweek_condition, 'group' => $groupby, 'order' => array('sum(ProcessedListing.price_per_product) desc'), 'limit' => 100));
					...
					$lastweek_Reports = $this->ProcessedListing->find('all',array('fields' => array('ProcessedListing.product_sku', 'ProcessedListing.product_name', 'ProcessedListing.cat_name','count(ProcessedListing.quantity) as sales_qty','sum(ProcessedListing.price_per_product) as sale_vale'), 'conditions' =>$lastweek_condition, 'group' => $groupby, 'order' => array('sum(ProcessedListing.price_per_product) desc'), 'limit' => 100));
                    ...
                    'ProcessedListing.order_date >= ' => $samemonth_lastweek,'ProcessedListing.cat_name !='=>'','ProcessedListing.price_per_product !='=>'0','ProcessedListing.currency !='=>'','ProcessedListing.plateform !='=>'','ProcessedListing.subsource !='=>'http://bhsindia.com','ProcessedListing.subsource !='=>'','ProcessedListing.subsource !='=>'http://dev.homescapesonline.com');
				 	...
				 	$lastmonth_Reports = $this->ProcessedListing->find('all',array('fields' => array('ProcessedListing.product_sku', 'ProcessedListing.product_name', 'ProcessedListing.cat_name','count(ProcessedListing.quantity) as sales_qty','sum(ProcessedListing.price_per_product) as sale_vale'), 'conditions' =>$lastmonth_condition, 'group' => $groupby, 'order' => array('sum(ProcessedListing.price_per_product) desc'), 'limit' => 100));
					...
					$lastyear_Reports = $this->ProcessedListing->find('all',array('fields' => array('ProcessedListing.product_sku', 'ProcessedListing.product_name', 'ProcessedListing.cat_name','count(ProcessedListing.quantity) as sales_qty','sum(ProcessedListing.price_per_product) as sale_vale'), 'conditions' =>$lastyear_condition, 'group' => $groupby, 'order' => array('sum(ProcessedListing.price_per_product) desc'), 'limit' => 100));
