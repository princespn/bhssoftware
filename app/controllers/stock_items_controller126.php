...
class StockItemsController extends AppController {
    	...
    	function beforeFilter() {
        	parent::beforeFilter();
        	$this->Auth->allow(array('minimum_level','dailyplateform_reports','dailysales_reports','category','stock_category', 'index'));
 ...
 public function categname() {
        ...
        return $catgory;
 		...
 		public function index(){
			...
			public function dailysales_reports(){
				
					$this->set('title', 'Daily Sales Reports - Top SKU Quantity Ordered.');
					...
					$last_monthdate = date('Y-m-d', strtotime("-1 months", strtotime($lastdate)));//2017-08-06
						
					$last_weekdate = date('Y-m-d', strtotime("-1 weeks", strtotime($lastdate)));//2017-08-06
					
					$last_daydate = date('Y-m-d', strtotime("-1 days", strtotime($lastdate)));//2017-08-06
										
			//	print_r($last_daydate);die();
					
					
					$six_condition = array('ProcessedListing.order_date <= ' => $lastdate,
                    'ProcessedListing.order_date >= ' => $six_firstdate,'ProcessedListing.cat_name !='=>'','ProcessedListing.price_per_product !='=>'0','ProcessedListing.currency !='=>'','ProcessedListing.plateform !='=>'','ProcessedListing.subsource !='=>'http://bhsindia.com','ProcessedListing.subsource !='=>'','ProcessedListing.subsource !='=>'http://dev.homescapesonline.com');
                    
					$sixmonth_Reports = $this->ProcessedListing->find('all',array('fields' => array('ProcessedListing.product_sku', 'ProcessedListing.product_name', 'ProcessedListing.cat_name','count(ProcessedListing.quantity) as sales_qty'), 'conditions' =>$six_condition, 'group' => $groupby, 'order' => array('count(ProcessedListing.quantity) desc'), 'limit' => 100));
					
					
					$lastday_condition = array('ProcessedListing.order_date' => $last_daydate,'ProcessedListing.cat_name !='=>'','ProcessedListing.price_per_product !='=>'0','ProcessedListing.currency !='=>'','ProcessedListing.plateform !='=>'','ProcessedListing.subsource !='=>'http://bhsindia.com','ProcessedListing.subsource !='=>'','ProcessedListing.subsource !='=>'http://dev.homescapesonline.com');
                    
					$lastday_Reports = $this->ProcessedListing->find('all',array('fields' => array('ProcessedListing.product_sku', 'ProcessedListing.product_name', 'ProcessedListing.cat_name','count(ProcessedListing.quantity) as sales_qty'), 'conditions' =>$lastday_condition, 'group' => $groupby, 'order' => array('count(ProcessedListing.quantity) desc'), 'limit' => 100));
					
					
					$lastweek_condition = array('ProcessedListing.order_date' => $last_weekdate,'ProcessedListing.cat_name !='=>'','ProcessedListing.price_per_product !='=>'0','ProcessedListing.currency !='=>'','ProcessedListing.plateform !='=>'','ProcessedListing.subsource !='=>'http://bhsindia.com','ProcessedListing.subsource !='=>'','ProcessedListing.subsource !='=>'http://dev.homescapesonline.com');
                 
					$lastweek_Reports = $this->ProcessedListing->find('all',array('fields' => array('ProcessedListing.product_sku', 'ProcessedListing.product_name', 'ProcessedListing.cat_name','count(ProcessedListing.quantity) as sales_qty'), 'conditions' =>$lastweek_condition, 'group' => $groupby, 'order' => array('count(ProcessedListing.quantity) desc'), 'limit' => 100));
					
					
					$lastmonth_condition = array('ProcessedListing.order_date' => $last_monthdate,'ProcessedListing.cat_name !='=>'','ProcessedListing.price_per_product !='=>'0','ProcessedListing.currency !='=>'','ProcessedListing.plateform !='=>'','ProcessedListing.subsource !='=>'http://bhsindia.com','ProcessedListing.subsource !='=>'','ProcessedListing.subsource !='=>'http://dev.homescapesonline.com');
                 
				    
					$lastmonth_Reports = $this->ProcessedListing->find('all',array('fields' => array('ProcessedListing.product_sku', 'ProcessedListing.product_name', 'ProcessedListing.cat_name','count(ProcessedListing.quantity) as sales_qty'), 'conditions' =>$lastmonth_condition, 'group' => $groupby, 'order' => array('count(ProcessedListing.quantity) desc'), 'limit' => 100));
					
					$lastyear_condition = array('ProcessedListing.order_date' => $last_yeardate,'ProcessedListing.cat_name !='=>'','ProcessedListing.price_per_product !='=>'0','ProcessedListing.currency !='=>'','ProcessedListing.plateform !='=>'','ProcessedListing.subsource !='=>'http://bhsindia.com','ProcessedListing.subsource !='=>'','ProcessedListing.subsource !='=>'http://dev.homescapesonline.com');
                 
					$lastyear_Reports = $this->ProcessedListing->find('all',array('fields' => array('ProcessedListing.product_sku', 'ProcessedListing.product_name', 'ProcessedListing.cat_name','count(ProcessedListing.quantity) as sales_qty'), 'conditions' =>$lastyear_condition, 'group' => $groupby, 'order' => array('count(ProcessedListing.quantity) desc'), 'limit' => 100));
				...
				//print_r($sixmonth_Reports);die();
					...
					$Cuurent_stocks = $this->StockLevel->find('all',array('fields' => array('StockLevel.item_number', 'StockLevel.stock_lev'), 'conditions' => array('StockLevel.location_name' =>'Default','StockLevel.change_date' => $currentdate), 'order' => array('StockLevel.item_number ASC')));
				
					$this->paginate = array('limit' => 100, 'order' => array('StockItem.item_number ASC'));
					$this->set('stock_names', $this->paginate()); 
					$this->set(compact('sixmonth_Reports','lastday_Reports','lastweek_Reports','lastmonth_Reports','lastyear_Reports','stock_names','Cuurent_stocks'));
			...
			public function dailyplateform_reports($last_sku){
					...
					$condition = array('ProcessedListing.product_sku' => $last_sku,'ProcessedListing.cat_name !='=>'','ProcessedListing.price_per_product !='=>'0','ProcessedListing.currency !='=>'','ProcessedListing.plateform !='=>'','ProcessedListing.subsource !='=>'http://bhsindia.com','ProcessedListing.subsource !='=>'','ProcessedListing.subsource !='=>'http://dev.homescapesonline.com');
                 
					$Reports = $this->ProcessedListing->find('all',array('fields' => array('ProcessedListing.product_sku', 'ProcessedListing.plateform', 'ProcessedListing.product_name', 'ProcessedListing.cat_name','count(ProcessedListing.quantity) as sales_qty'), 'conditions' =>$condition, 'group' => $groupby, 'order' => array('count(ProcessedListing.quantity) desc')));
					
					
					$yes_date = date('Y-m-d', strtotime("-1 days", strtotime($lastdate)));//2017-08-06
										
					
					$yes_condition = array('ProcessedListing.product_sku' => $last_sku, 'ProcessedListing.product_sku' => $yes_date, 'ProcessedListing.cat_name !='=>'','ProcessedListing.price_per_product !='=>'0','ProcessedListing.currency !='=>'','ProcessedListing.plateform !='=>'','ProcessedListing.subsource !='=>'http://bhsindia.com','ProcessedListing.subsource !='=>'','ProcessedListing.subsource !='=>'http://dev.homescapesonline.com');
                 
					$yes_Reports = $this->ProcessedListing->find('all',array('fields' => array('ProcessedListing.product_sku', 'ProcessedListing.plateform', 'ProcessedListing.product_name', 'ProcessedListing.cat_name','count(ProcessedListing.quantity) as sales_qty'), 'conditions' =>$yes_condition, 'group' => $groupby, 'order' => array('count(ProcessedListing.quantity) desc')));
					
					
					
					$lastseven_date = date('Y-m-d', strtotime("-7 days", strtotime($lastdate)));//2017-08-06
										
					
					$lastseven_condition = array('ProcessedListing.product_sku' => $last_sku, 'ProcessedListing.product_sku' => $lastseven_date, 'ProcessedListing.cat_name !='=>'','ProcessedListing.price_per_product !='=>'0','ProcessedListing.currency !='=>'','ProcessedListing.plateform !='=>'','ProcessedListing.subsource !='=>'http://bhsindia.com','ProcessedListing.subsource !='=>'','ProcessedListing.subsource !='=>'http://dev.homescapesonline.com');
                 
					$lastseven_Reports = $this->ProcessedListing->find('all',array('fields' => array('ProcessedListing.product_sku', 'ProcessedListing.plateform', 'ProcessedListing.product_name', 'ProcessedListing.cat_name','count(ProcessedListing.quantity) as sales_qty'), 'conditions' =>$lastseven_condition, 'group' => $groupby, 'order' => array('count(ProcessedListing.quantity) desc')));
					
					
					
					$lasttherty_date = date('Y-m-d', strtotime("-30 days", strtotime($lastdate)));//2017-08-06
										
					
					$lasttherty_condition = array('ProcessedListing.product_sku' => $last_sku, 'ProcessedListing.product_sku' => $lasttherty_date, 'ProcessedListing.cat_name !='=>'','ProcessedListing.price_per_product !='=>'0','ProcessedListing.currency !='=>'','ProcessedListing.plateform !='=>'','ProcessedListing.subsource !='=>'http://bhsindia.com','ProcessedListing.subsource !='=>'','ProcessedListing.subsource !='=>'http://dev.homescapesonline.com');
                 
					$lasttherty_Reports = $this->ProcessedListing->find('all',array('fields' => array('ProcessedListing.product_sku', 'ProcessedListing.plateform', 'ProcessedListing.product_name', 'ProcessedListing.cat_name','count(ProcessedListing.quantity) as sales_qty'), 'conditions' =>$lasttherty_condition, 'group' => $groupby, 'order' => array('count(ProcessedListing.quantity) desc')));
					//print_r();die();
					
					$lastninty_date = date('Y-m-d', strtotime("-90 days", strtotime($lastdate)));//2017-08-06
										
					
					$lastninty_condition = array('ProcessedListing.product_sku' => $last_sku, 'ProcessedListing.product_sku' => $lastninty_date, 'ProcessedListing.cat_name !='=>'','ProcessedListing.price_per_product !='=>'0','ProcessedListing.currency !='=>'','ProcessedListing.plateform !='=>'','ProcessedListing.subsource !='=>'http://bhsindia.com','ProcessedListing.subsource !='=>'','ProcessedListing.subsource !='=>'http://dev.homescapesonline.com');
                 
					$lastninty_Reports = $this->ProcessedListing->find('all',array('fields' => array('ProcessedListing.product_sku', 'ProcessedListing.plateform', 'ProcessedListing.product_name', 'ProcessedListing.cat_name','count(ProcessedListing.quantity) as sales_qty'), 'conditions' =>$lastninty_condition, 'group' => $groupby, 'order' => array('count(ProcessedListing.quantity) desc')));
					//print_r();die();
				
					$last365_date = date('Y-m-d', strtotime("-365 days", strtotime($lastdate)));//2017-08-06
										
					
					$last365_condition = array('ProcessedListing.product_sku' => $last_sku, 'ProcessedListing.product_sku' => $last365_date, 'ProcessedListing.cat_name !='=>'','ProcessedListing.price_per_product !='=>'0','ProcessedListing.currency !='=>'','ProcessedListing.plateform !='=>'','ProcessedListing.subsource !='=>'http://bhsindia.com','ProcessedListing.subsource !='=>'','ProcessedListing.subsource !='=>'http://dev.homescapesonline.com');
                 
					$last365_Reports = $this->ProcessedListing->find('all',array('fields' => array('ProcessedListing.product_sku', 'ProcessedListing.plateform', 'ProcessedListing.product_name', 'ProcessedListing.cat_name','count(ProcessedListing.quantity) as sales_qty'), 'conditions' =>$last365_condition, 'group' => $groupby, 'order' => array('count(ProcessedListing.quantity) desc')));
					//print_r();die();
