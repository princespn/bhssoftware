    ...
    public function repdelcode($filename)
    {
        ...
        while (($row = fgetcsv($handle)) !== FALSE) {
           ...
           // print_r($data['MasterListing']['new_amazon_sku']);die();
            ...
            $newsku = isset($row[1]) ? $row[1] : 1;

            if (!empty($id)) {
                $pcodes = $this->find('all', array('conditions' => array('MasterListing.amazon_sku' => $id)));


                if (((!empty($data['MasterListing']['amazon_sku'])) && (empty($pcodes[0]['MasterListing']['sale_price_fr'])) && (empty($pcodes[0]['MasterListing']['sale_price_de']))) && ($id === $pcodes[0]['MasterListing']['amazon_sku'])) {

                    $condition = array('MasterListing.id' => $pcodes[0]['MasterListing']['id']);
                    $this->deleteAll($condition, false);
                   // print_r($this->deleteAll($condition, false));
                   // die();
                    $limit = 'Amazon SKU->' . $id . 'Deleted Sucessfully';
                    $return['errors'][] = __(sprintf("Amazon sku $id  deleted :$limit.", $i), true);
                    $erritem[] = __(sprintf("Amazon sku $id  deleted :$limit.", $i), true);
                    $err = implode("\n", $erritem);
                    $this->saveField('error', $err, array($this->amazon_sku = $id));

                }

                if ((!empty($data['MasterListing']['amazon_sku'])) && (!empty($data['MasterListing']['new_amazon_sku'])) && ($id === $pcodes[0]['MasterListing']['amazon_sku'])) {

                   // $condition = array('MasterListing.id' => $pcodes[0]['MasterListing']['id']);
                   // $this->updateAll($condition, false);

                    $db = $this->getDataSource();
                    $value = $db->value($newsku, 'string');
                    $this->updateAll(
                        array('MasterListing.amazon_sku' => $value),
                        array('MasterListing.amazon_sku' => $id )
                    );


                    //print_r($this->updateAll(array('MasterListing.amazon_sku' => "'$newsku'"), array('MasterListing.amazon_sku'=>"'$id'",'MasterListing.linnworks_code' => $pcodes[0]['MasterListing']['linnworks_code'])));

                    // print_r($this->updateAll(array('MasterListing.amazon_sku'=> $newsku), array('MasterListing.amazon_sku' => $id, 'MasterListing.linnworks_code' => $pcodes[0]['MasterListing']['linnworks_code'])));

                    $limit = 'Amazon SKU ' . $id . ' Updated Sucessfully ';
                    $return['errors'][] = __(sprintf(" Amazon SKU $id  Updated :$limit.", $i), true);
                    $erritem[] = __(sprintf("Amazon SKU $id  Updated :$limit.", $i), true);
                    $err = implode("\n", $erritem);
                    $this->saveField('error', $err, array($pcodes[0]['MasterListing']['amazon_sku'] = $id));
        ...
        while (($row = fgetcsv($handle)) !== FALSE) {
            ...
            if (!empty($id)) {
                ...
                if ((!empty($pcodes))) {
                    ...
                    if(((strpos($data['MasterListing']['amazon_sku'], 'FBA') === false)  && (!empty($WebConfig['web_sale_price_tesco'])) && (!empty($data['MasterListing']['sale_price_uk']))) && (($WebConfig['web_sale_price_tesco'])!==($data['MasterListing']['sale_price_uk']))) {
                        $limit = 'Web Tesco and Amazon UK Price Mismatch.';
                    ...
                    if(((strpos($data['MasterListing']['amazon_sku'], 'FBA') === false)  && (!empty($WebConfig['web_sale_price_uk'])) && (!empty($data['MasterListing']['sale_price_uk']))) && (($WebConfig['web_sale_price_uk'])!==($data['MasterListing']['sale_price_uk']))) {
                    ...
                    if(((strpos($data['MasterListing']['amazon_sku'], 'FBA') !== false)  && (!empty($WebConfig['web_sale_price_dm'])) && (!empty($data['MasterListing']['sale_price_uk']))) && (($WebConfig['web_sale_price_dm'])!==($data['MasterListing']['sale_price_uk']))) {
                    ...
                    if(((strpos($data['MasterListing']['amazon_sku'], 'FBA') === false)  && (!empty($WebConfig['web_sale_price_de'])) && (!empty($data['MasterListing']['sale_price_de']))) && (($WebConfig['web_sale_price_de'])!==($data['MasterListing']['sale_price_de']))) {
                    ...
                    if(((strpos($data['MasterListing']['amazon_sku'], 'FBA') === false)  && (!empty($WebConfig['web_sale_price_fr'])) && (!empty($data['MasterListing']['sale_price_fr']))) && (($WebConfig['web_sale_price_fr'])!==($data['MasterListing']['sale_price_fr']))) {
