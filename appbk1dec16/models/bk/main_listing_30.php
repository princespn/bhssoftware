...
class MainListing extends AppModel {
    ...
    public function importcode($filename) {
            ...
            if (!empty($id)) {
                ...
                if ((!empty($pcodes))) {
                    ...
                    $WebConfig = (isset($Webcodes[0]['Listing']) && is_array($Webcodes[0]['Listing'])) ? ($Webcodes[0]['Listing']) : array();
                     ...
                     if ((($itemsku[1]) !== 'FBA') && ((!empty($apiConfig['price_uk'])) && (!empty($WebConfig['web_price_uk'])) && (!empty($data['MainListing']['price_uk'])) && (($WebConfig['web_price_uk']) !== ($data['MainListing']['price_uk'])))) {
                        //$data['MainListing'] = array_merge($apiConfig['price_uk'], $data['MainListing']['price_uk']);
                        $limit = 'RRP  Web UK and Amazon UK Price Mismatch.';
                    ...
                    if ((($itemsku[1]) !== 'FBA') && (($final) !== 'FBA') && ((!empty($apiConfig['sale_price_uk'])) && (!empty($WebConfig['web_sale_price_uk'])) && (!empty($data['MainListing']['sale_price_uk'])) && (($WebConfig['web_sale_price_uk']) !== ($data['MainListing']['sale_price_uk'])))) {
                        // $data['MainListing'] = array_merge($apiConfig['price_uk'], $data['MainListing']['price_uk']);
                    ...
                    if((($itemsku[1]) === 'FBA') && (!empty($apiConfig['sale_price_uk'])) && (!empty($data['MainListing']['sale_price_uk'])) && (!empty($WebConfig['web_sale_price_uk']))){
                     ...
                     if($val >= $min && $val <= $max){ }else{
                         $limit = 'Amazon Prim Min Price '.$min.'And Max Price '.$max;
                        $return['errors'][] = __(sprintf("Listing Could not be processed due to error on line %d and Item sku $id :$limit.", $i), true);
                        $erritem[] = __(sprintf("Listing Could not be processed due to error on line %d and Item sku $id :$limit.", $i), true);
                        $err = implode("\n", $erritem);
                        $this->saveField('error', $err, array($this->id = $i));        
                    ...
                    if((($itemsku[1]) === 'FBA') && (!empty($apiConfig['sale_price_uk'])) && (!empty($data['MainListing']['sale_price_uk'])) && (!empty($WebConfig['web_sale_price_dm'])) && (($WebConfig['web_sale_price_dm'])!==($data['MainListing']['sale_price_uk']))) { 
                         //  $data['MainListing'] = array_merge($apiConfig['sale_price_uk'], $data['MainListing']['sale_price_uk']);
       ...
       // GBP Codition End and Start EUR Condition
                    ...
                    if((($itemsku[1]) !== 'FBA') && (($final) !== 'FBA') && (!empty($apiConfig['sale_price_uk'])) && (!empty($WebConfig['web_sale_price_uk'])) && (!empty($WebConfig['web_sale_price_de']))){
                        //$data['MainListing'] = array_merge($apiConfig['sale_price_uk'], $data['MainListing']['sale_price_uk']);
                    ...
                    if((($itemsku[1]) !== 'FBA') && (($final) !== 'FBA') && (!empty($apiConfig['price_uk'])) && (!empty($data['MainListing']['sale_price_de'])) && (!empty($WebConfig['web_sale_price_de']))){
                    ...
                    /*
                     if((($itemsku[1]) !== 'FBA') && (($final) !== 'FBA') && ((!empty($apiConfig['price_fr'])) && (!empty($data['MainListing']['price_fr'])) && (!empty($WebConfig['web_price_fr'])) && (($data['MainListing']['price_fr'])!==($WebConfig['web_price_fr'])))){
     
                        $data['MainListing'] = array_merge($apiConfig['price_fr'], $data['MainListing']['price_fr']);
                        $limit = 'RRP  Web FR and Amazon FR Price Mismatch.';
                        $return['errors'][] = __(sprintf("Listing Could not be processed due to error on line %d and Item sku $id :$limit.", $i), true);
                        $erritem[] = __(sprintf("Listing Could not be processed due to error on line %d and Item sku $id :$limit.", $i), true);
                        $err = implode("\n", $erritem);
                        $this->saveField('error', $err, array($this->id = $i));
                    }
                    if((($itemsku[1]) !== 'FBA') && (($final) !== 'FBA') && (!empty($apiConfig['price_de'])) && (!empty($data['MainListing']['price_de'])) && (!empty($WebConfig['web_price_de'])) && (($data['MainListing']['price_de'])!==($WebConfig['web_price_de']))){
     
                        $data['MainListing'] = array_merge($apiConfig['price_de'], $data['MainListing']['price_de']);
                        $limit = 'RRP  Web DE and Amazon DE Price Mismatch.';
                        $return['errors'][] = __(sprintf("Listing Could not be processed due to error on line %d and Item sku $id :$limit.", $i), true);
                        $erritem[] = __(sprintf("Listing Could not be processed due to error on line %d and Item sku $id :$limit.", $i), true);
                        $err = implode("\n", $erritem);
                        $this->saveField('error', $err, array($this->id = $i));
