...
/**
 * GermanMasterListings Model
 *
 * @property \App\Model\Table\UsersTable|\Cake\ORM\Association\BelongsTo $Users
 * @property \App\Model\Table\ExternalProductsTable|\Cake\ORM\Association\BelongsTo $ExternalProducts
 * @property \App\Model\Table\DeliveryScheduleGroupsTable|\Cake\ORM\Association\BelongsTo $DeliveryScheduleGroups
 * @property \App\Model\Table\FulfillmentCentersTable|\Cake\ORM\Association\BelongsTo $FulfillmentCenters
 * @property \App\Model\Table\FedasTable|\Cake\ORM\Association\BelongsTo $Fedas
 *
 * @method \App\Model\Entity\GermanMasterListing get($primaryKey, $options = [])
 * @method \App\Model\Entity\GermanMasterListing newEntity($data = null, array $options = [])
 * @method \App\Model\Entity\GermanMasterListing[] newEntities(array $data, array $options = [])
 * @method \App\Model\Entity\GermanMasterListing|bool save(\Cake\Datasource\EntityInterface $entity, $options = [])
 * @method \App\Model\Entity\GermanMasterListing patchEntity(\Cake\Datasource\EntityInterface $entity, array $data, array $options = [])
 * @method \App\Model\Entity\GermanMasterListing[] patchEntities($entities, array $data, array $options = [])
 * @method \App\Model\Entity\GermanMasterListing findOrCreate($search, callable $callback = null, $options = [])
 */
class GermanMasterListingsTable extends Table
{
    ...
    public function initialize(array $config)
    {
        ...
        $this->setTable('german_master_listings');
        ...
        $this->belongsTo('DeliveryScheduleGroups', [
            'foreignKey' => 'delivery_schedule_group_id'
        ]);
        $this->belongsTo('FulfillmentCenters', [
            'foreignKey' => 'fulfillment_center_id'
        ]);
        $this->belongsTo('Fedas', [
            'foreignKey' => 'fedas_id'
        ]);
    }
    ...
    public function validationDefault(Validator $validator)
    {
        ...
        $validator
            ...
            ->maxLength('max_aggregate_ship_quantity', 100)
            ->allowEmpty('max_aggregate_ship_quantity');

        $validator
            ->scalar('max_order_quantity')
            ->maxLength('max_order_quantity', 100)
            ->allowEmpty('max_order_quantity');
        ...
        $validator
            ...
            ->maxLength('product_tax_code', 100)
            ->allowEmpty('product_tax_code');

        $validator
            ->scalar('merchant_shipping_group_name')
            ->maxLength('merchant_shipping_group_name', 400)
            ->allowEmpty('merchant_shipping_group_name');

        $validator
            ->scalar('website_shipping_weight')
            ->maxLength('website_shipping_weight', 200)
            ->allowEmpty('website_shipping_weight');

        $validator
            ->scalar('website_shipping_weight_unit_of_measure')
            ->maxLength('website_shipping_weight_unit_of_measure', 200)
            ->allowEmpty('website_shipping_weight_unit_of_measure');

        $validator
            ->scalar('item_weight')
            ->maxLength('item_weight', 200)
            ->allowEmpty('item_weight');

        $validator
            ->scalar('item_weight_unit_of_measure')
            ->maxLength('item_weight_unit_of_measure', 200)
            ->allowEmpty('item_weight_unit_of_measure');

        $validator
            ->scalar('item_length')
            ->maxLength('item_length', 200)
            ->allowEmpty('item_length');

        $validator
            ->scalar('item_length_unit_of_measure')
            ->maxLength('item_length_unit_of_measure', 200)
            ->allowEmpty('item_length_unit_of_measure');

        $validator
            ->scalar('item_width')
            ->maxLength('item_width', 200)
            ->allowEmpty('item_width');

        $validator
            ->scalar('item_width_unit_of_measure')
            ->maxLength('item_width_unit_of_measure', 200)
            ->allowEmpty('item_width_unit_of_measure');

        $validator
            ->scalar('item_height')
            ->maxLength('item_height', 200)
            ->allowEmpty('item_height');

        $validator
            ->scalar('item_height_unit_of_measure')
            ->maxLength('item_height_unit_of_measure', 200)
            ->allowEmpty('item_height_unit_of_measure');
        ...
        $validator
            ...
            ->maxLength('item_display_diameter', 300)
            ->allowEmpty('item_display_diameter');

        $validator
            ->scalar('item_display_diameter_unit_of_measure')
            ->maxLength('item_display_diameter_unit_of_measure', 200)
        ...
        $validator
            ->scalar('target_audience_keywords')
            ->allowEmpty('target_audience_keywords');
        ...
        $validator
            ->scalar('package_weight')
            ->maxLength('package_weight', 100)
            ->allowEmpty('package_weight');

        $validator
            ->scalar('package_weight_unit_of_measure')
            ->maxLength('package_weight_unit_of_measure', 100)
            ->allowEmpty('package_weight_unit_of_measure');
        ...
        $validator
            ->scalar('scent_name')
            ->maxLength('scent_name', 100)
            ->allowEmpty('scent_name');

        $validator
            ->scalar('is_stain_resistant')
            ->maxLength('is_stain_resistant', 100)
            ->allowEmpty('is_stain_resistant');

        $validator
            ->scalar('color_name')
            ->maxLength('color_name', 100)
            ->allowEmpty('color_name');

        $validator
            ->scalar('size_name')
            ->maxLength('size_name', 100)
            ->allowEmpty('size_name');

        $validator
            ->scalar('thread_count')
            ->maxLength('thread_count', 100)
            ->allowEmpty('thread_count');

        $validator
            ->scalar('material_type')
            ->maxLength('material_type', 100)
            ->allowEmpty('material_type');
        ...
        $validator
            ->scalar('wattage')
            ->maxLength('wattage', 100)
            ->allowEmpty('wattage');

        $validator
            ->scalar('unit_count')
            ->maxLength('unit_count', 200)
            ->allowEmpty('unit_count');

        $validator
            ->scalar('unit_count_type')
            ->maxLength('unit_count_type', 200)
            ->allowEmpty('unit_count_type');

        $validator
            ->scalar('thermal_performance_description')
            ->maxLength('thermal_performance_description', 100)
            ->allowEmpty('thermal_performance_description');

        $validator
            ->scalar('special_features')
            ->maxLength('special_features', 300)
            ->allowEmpty('special_features');

        $validator
            ->scalar('seasons1')
            ->maxLength('seasons1', 300)
            ->allowEmpty('seasons1');

        $validator
            ->scalar('seasons2')
            ->maxLength('seasons2', 300)
            ->allowEmpty('seasons2');

        $validator
            ->scalar('seasons3')
            ->maxLength('seasons3', 300)
            ->allowEmpty('seasons3');

        $validator
            ->scalar('seasons4')
            ->maxLength('seasons4', 300)
            ->allowEmpty('seasons4');

        $validator
            ->scalar('outer_material_type1')
            ->maxLength('outer_material_type1', 100)
            ->allowEmpty('outer_material_type1');

        $validator
            ->scalar('outer_material_type2')
            ->maxLength('outer_material_type2', 100)
            ->allowEmpty('outer_material_type2');

        $validator
            ->scalar('outer_material_type3')
            ->maxLength('outer_material_type3', 100)
            ->allowEmpty('outer_material_type3');

        $validator
            ->scalar('outer_material_type4')
            ->maxLength('outer_material_type4', 100)
            ->allowEmpty('outer_material_type4');

        $validator
            ->scalar('outer_material_type5')
            ->maxLength('outer_material_type5', 100)
            ->allowEmpty('outer_material_type5');

        $validator
            ->scalar('occupancy')
            ->maxLength('occupancy', 100)
            ->allowEmpty('occupancy');

        $validator
            ->scalar('mfg_minimum')
            ->maxLength('mfg_minimum', 100)
            ->allowEmpty('mfg_minimum');

        $validator
            ->scalar('material_composition')
            ->maxLength('material_composition', 100)
            ->allowEmpty('material_composition');

        $validator
            ->scalar('item_type_name')
            ->maxLength('item_type_name', 100)
            ->allowEmpty('item_type_name');

        $validator
            ->scalar('item_thickness_derived')
            ->maxLength('item_thickness_derived', 100)
            ->allowEmpty('item_thickness_derived');

        $validator
            ->scalar('item_thickness_unit_of_measure')
            ->maxLength('item_thickness_unit_of_measure', 100)
            ->allowEmpty('item_thickness_unit_of_measure');

        $validator
            ->scalar('item_shape')
            ->maxLength('item_shape', 100)
            ->allowEmpty('item_shape');

        $validator
            ->scalar('inner_material_type')
            ->maxLength('inner_material_type', 100)
            ->allowEmpty('inner_material_type');
        ...
        $validator
            ->scalar('capacity_unit_of_measure')
            ->maxLength('capacity_unit_of_measure', 100)
            ->allowEmpty('capacity_unit_of_measure');

        $validator
            ->scalar('are_batteries_included')
            ->maxLength('are_batteries_included', 100)
            ->allowEmpty('are_batteries_included');

        $validator
            ->scalar('batteries_required')
            ->maxLength('batteries_required', 100)
            ->allowEmpty('batteries_required');

        $validator
            ->scalar('battery_type1')
            ->maxLength('battery_type1', 100)
            ->allowEmpty('battery_type1');

        $validator
            ->scalar('battery_type2')
            ->maxLength('battery_type2', 100)
            ->allowEmpty('battery_type2');

        $validator
            ->scalar('battery_type3')
            ->maxLength('battery_type3', 100)
            ->allowEmpty('battery_type3');

        $validator
            ->scalar('number_of_batteries1')
            ->maxLength('number_of_batteries1', 100)
            ->allowEmpty('number_of_batteries1');

        $validator
            ->scalar('number_of_batteries2')
            ->maxLength('number_of_batteries2', 100)
            ->allowEmpty('number_of_batteries2');

        $validator
            ->scalar('number_of_batteries3')
            ->maxLength('number_of_batteries3', 100)
            ->allowEmpty('number_of_batteries3');

        $validator
            ->scalar('efficiency')
            ->maxLength('efficiency', 100)
        ...
        $validator
            ->scalar('style_name')
            ->maxLength('style_name', 100)
            ->allowEmpty('style_name');

        $validator
            ->scalar('specific_uses_for_product')
            ->maxLength('specific_uses_for_product', 100)
            ->allowEmpty('specific_uses_for_product');

        $validator
            ->scalar('seating_capacity')
            ->maxLength('seating_capacity', 100)
            ->allowEmpty('seating_capacity');

        $validator
            ->scalar('pattern_name')
            ->maxLength('pattern_name', 100)
            ->allowEmpty('pattern_name');

        $validator
            ->scalar('paper_size')
            ->maxLength('paper_size', 100)
            ->allowEmpty('paper_size');

        $validator
            ->scalar('paint_type')
            ->maxLength('paint_type', 100)
            ->allowEmpty('paint_type');

        $validator
            ->scalar('occasion_type')
            ->maxLength('occasion_type', 100)
            ->allowEmpty('occasion_type');

        $validator
            ->scalar('number_of_doors')
            ->maxLength('number_of_doors', 100)
            ->allowEmpty('number_of_doors');

        $validator
            ->scalar('line_weight')
            ->maxLength('line_weight', 100)
            ->allowEmpty('line_weight');

        $validator
            ->scalar('item_styling')
            ->maxLength('item_styling', 100)
            ->allowEmpty('item_styling');

        $validator
            ->scalar('item_hardness')
            ->maxLength('item_hardness', 100)
            ->allowEmpty('item_hardness');

        $validator
            ->scalar('adjustment_type')
            ->maxLength('adjustment_type', 100)
            ->allowEmpty('adjustment_type');

        $validator
            ->scalar('installation_type')
            ->maxLength('installation_type', 100)
            ->allowEmpty('installation_type');

        $validator
            ->scalar('included_components')
            ->maxLength('included_components', 100)
            ->allowEmpty('included_components');

        $validator
            ->scalar('frame_type')
            ->maxLength('frame_type', 100)
            ->allowEmpty('frame_type');

        $validator
            ->scalar('form_factor')
            ->maxLength('form_factor', 100)
    ...
    public function buildRules(RulesChecker $rules)
    {
        ...
        $rules->add($rules->existsIn(['delivery_schedule_group_id'], 'DeliveryScheduleGroups'));
        $rules->add($rules->existsIn(['fulfillment_center_id'], 'FulfillmentCenters'));
        $rules->add($rules->existsIn(['fedas_id'], 'Fedas'));
